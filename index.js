...
function getCarInfoById(inventory, number) {
    ...
    for (let x = 0; x < inventory.length; x++) {
        if (inventory[x].id === number) return (`This is a ${inventory[x].car_make} ${inventory[x].car_model}`);
    }
    return null;
}
...
function sortCarInventory( /* code here */ inventory) {
    ...
    return inventory.sort(function(a, b) {
        let car_modelA = a.car_model.toLowerCase(),
            car_modelB = b.car_model.toLowerCase();
 ...
 */
function carMaker( /* code here */ _odometer) {
    /* code here */
    return {
        odometer: _odometer,
        drive(distance) {
